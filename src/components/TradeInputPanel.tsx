  ...
  return (
    <>
      <section className="flex flex-col gap-3 rounded-lg border border-white/10 bg-[#101217] p-3 shadow-2xl shadow-black/30 lg:min-h-full lg:gap-2.5">
        ...
        <FileDrop
          title={t("Скриншоты сделки")}
          files={files}
          inputName="trade"
          accent="emerald"
          description={t("Загрузи все фото одной связки: фьючерсы, спот, балансы, ордера, депозиты и выводы.")}
        ...
        <label className="flex flex-1 flex-col gap-2">
          <span className="flex items-center gap-2 text-sm font-medium text-[#dce2ea] lg:text-[13px]">
            ...
            {t("Условия и корректировки")}
          </span>
          <textarea
            value={instructions}
            onChange={(event) => onInstructionsChange(event.target.value)}
            placeholder={t("Например: спот вышел в -16.9 USDT")}
              ...
              <div className="flex min-w-0 items-start gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-sm text-[#aeb7c3]">
                    {t("Полный текст ошибки можно скопировать.")}
                  </p>
                </div>
              </div>
              <button
